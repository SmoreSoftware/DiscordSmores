...
module.exports = class OrderCommand extends commando.Command {
  ...
  async run(message, args) {
    ...
    activeOrders[message.author.id]
    if (activeOrders.includes(message.author.id)) return message.reply('You already have an order!')
      ...
      if (reason === 'aborted') {
        ...
        ordersChan.send({
          ...
          function deliver() {
            ...
            //eslint-disable-next-line no-mixed-operators
            let time2 = (Math.random() * (max - min) + min).toFixed(1)
            time2 = time2.replace('.', '')
            time2 += '0000'
            console.log(time2)
            ...
            function sendToCustomer() {
              ...
              if (args.toOrder.toLowerCase().includes('smore 1') || args.toOrder.toLowerCase().includes('s\'more 1') || args.toOrder.toLowerCase().includes('smores 1') || args.toOrder.toLowerCase().includes('s\'mores 1')) {
                ...
                delete activeOrders[message.author.id]
